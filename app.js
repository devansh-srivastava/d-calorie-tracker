(() => {
  ...
  async function syncDashboard() { const remote = window.APEX_CONFIG || {}; if (!remote.API_URL || !remote.API_SECRET) return; try { const url = new URL(remote.API_URL); url.searchParams.set('action', 'dashboard'); url.searchParams.set('secret', remote.API_SECRET); const response = await fetch(url, { cache: 'no-store' }); const data = await response.json(); if (!data.ok) return; state.settings = { ...DEFAULT_SETTINGS, ...(data.settings || {}) }; state.days ||= {}; (data.meals || []).forEach((meal) => { const day = state.days[meal.date] ||= { meals: [], exercises: {}, weight: null }; day.meals.push(meal); }); (data.weights || []).forEach((reading) => { (state.days[reading.date] ||= { meals: [], exercises: {}, weight: null }).weight = Number(reading.value); }); (data.exercises || []).forEach((exercise) => { const day = state.days[exercise.date] ||= { meals: [], exercises: {}, weight: null }; day.exercises[exercise.name] = Boolean(exercise.completed); }); render(); } catch { /* local data remains available */ } }
